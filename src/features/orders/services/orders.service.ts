    ...
    items: [
      {
        ...
        total: 34.0,
        modifiers: [
          { id: "mod-1", name: "حجم كبير", nameEn: "Large", price: 2.0 },
        ],
      },
      {
        id: "item-2",
...
export class OrdersService {
  ...
  static async getOrders(filters?: OrderFilters): Promise<Order[]> {
    ...
    if (filters) {
      ...
      if (filters.dateFrom) {
        filteredOrders = filteredOrders.filter(
          (order) => order.createdAt >= filters.dateFrom!
        );
      }
      if (filters.dateTo) {
        filteredOrders = filteredOrders.filter(
          (order) => order.createdAt <= filters.dateTo!
        );
      }
    }
  ...
  static async getOrderActivities(orderId: string): Promise<OrderActivity[]> {
    await new Promise((resolve) => setTimeout(resolve, 200));
    return MOCK_ORDER_ACTIVITIES.filter(
      (activity) => activity.orderId === orderId
