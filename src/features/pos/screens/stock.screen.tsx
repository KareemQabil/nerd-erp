...
import { Product, Warehouse } from '../types/pos.types';
import { POSService, WarehouseService } from '../services/pos.service';
import { ArrowRight, Package, AlertTriangle, Search, Warehouse as WarehouseIcon } from 'lucide-react';
        ...
        <div className="container mx-auto px-4">
          <div className="h-18 flex items-center justify-between">
            ...
            >
              ...
              <span className="font-['Almarai'] text-[#c2c7ce]" dir="auto">رجوع</span>
            </button>

            <h1 className="text-2xl font-['Almarai'] font-bold text-[#e2e2e6]" dir="auto">
              {t('pos.stockStatus')}
            </h1>
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          <div className="bg-gradient-to-b from-[#1a1c1e] via-[#1d2222] to-[#42474e] rounded-2xl p-4 border border-[#42474e]">
            ...
            <p className="text-sm font-['Almarai'] text-[#c2c7ce] mb-1" dir="auto">إجمالي المنتجات</p>
            <p className="text-2xl font-['Arial'] font-bold text-[#e2e2e6]">{stats.total}</p>
          </div>
          ...
          <div className="bg-gradient-to-b from-[rgba(245,158,11,0.2)] to-[rgba(217,119,6,0.2)] rounded-2xl p-4 border border-[#f59e0b]">
            ...
            <p className="text-sm font-['Almarai'] text-[#c2c7ce] mb-1" dir="auto">مخزون منخفض</p>
            <p className="text-2xl font-['Arial'] font-bold text-[#f59e0b]">{stats.low}</p>
          </div>
          ...
          <div className="bg-gradient-to-b from-[rgba(239,68,68,0.2)] to-[rgba(220,38,38,0.2)] rounded-2xl p-4 border border-[#ef4444]">
            ...
            <p className="text-sm font-['Almarai'] text-[#c2c7ce] mb-1" dir="auto">نفد المخزون</p>
            <p className="text-2xl font-['Arial'] font-bold text-[#ef4444]">{stats.out}</p>
          </div>
          ...
          <div className="bg-gradient-to-b from-[rgba(34,211,238,0.2)] to-[rgba(0,99,153,0.2)] rounded-2xl p-4 border border-cyan-400">
            ...
            <p className="text-sm font-['Almarai'] text-[#c2c7ce] mb-1" dir="auto">قيمة المخزون</p>
            <p className="text-2xl font-['Arial'] font-bold text-cyan-400">{stats.totalValue.toFixed(0)}</p>
          </div>
        </div>

        {/* Warehouses */}
        <div className="mb-6">
          <h2 className="text-lg font-['Almarai'] font-bold text-[#e2e2e6] mb-3" dir="auto">
            المستودعات
          </h2>
            ...
            {warehouses.map((wh) => (
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <h3 className="text-base font-['Almarai'] font-bold text-[#e2e2e6]" dir="auto">
                      {wh.name}
                    </h3>
                    <p className="text-xs text-[#c2c7ce] font-['Inter']">{wh.nameEn}</p>
                  </div>
                ...
                <div className="grid grid-cols-4 gap-2">
                  {wh.zones.map((zone) => {
                    const utilization = ((zone.currentStock / zone.capacity) * 100).toFixed(0);
                    return (
                      <div
                        key={zone.id}
                        className="bg-[rgba(255,255,255,0.05)] rounded-lg p-2 text-center"
                      >
                        <p className="text-xs text-[#c2c7ce] font-['Almarai'] mb-1" dir="auto">
                          {zone.name}
                        </p>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-[rgba(255,255,255,0.05)] border-b border-[rgba(255,255,255,0.1)]">
                  <tr>
                    <th className="px-6 py-4 text-right text-sm font-['Almarai'] text-[#c2c7ce]" dir="auto">المنتج</th>
                    <th className="px-6 py-4 text-center text-sm font-['Almarai'] text-[#c2c7ce]" dir="auto">SKU</th>
                    <th className="px-6 py-4 text-center text-sm font-['Almarai'] text-[#c2c7ce]" dir="auto">الفئة</th>
                    <th className="px-6 py-4 text-center text-sm font-['Almarai'] text-[#c2c7ce]" dir="auto">الكمية</th>
                    <th className="px-6 py-4 text-center text-sm font-['Almarai'] text-[#c2c7ce]" dir="auto">السعر</th>
                    <th className="px-6 py-4 text-center text-sm font-['Almarai'] text-[#c2c7ce]" dir="auto">القيمة</th>
                    <th className="px-6 py-4 text-center text-sm font-['Almarai'] text-[#c2c7ce]" dir="auto">الحالة</th>
                  </tr>
                </thead>
                <tbody>
                      ...
                      >
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-sm font-['Almarai'] font-bold text-[#e2e2e6]" dir="auto">
                                {product.name}
                              </p>
                        ...
                        <td className="px-6 py-4 text-center">
                          <span className={`text-lg font-['Arial'] font-bold ${
                            status === 'out' ? 'text-[#ef4444]' :
                            status === 'low' ? 'text-[#f59e0b]' :
                            'text-[#e2e2e6]'
                          }`}>
                            {product.stock}
                          </span>
                        ...
                        <td className="px-6 py-4 text-center">
                          <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full border text-xs font-['Almarai'] ${getStockColor(status)}`}>
                            <span className="w-2 h-2 rounded-full bg-current" />
                            <span dir="auto">
                              {status === 'out' ? 'نفد' :
                               status === 'low' ? 'منخفض' :
                               status === 'medium' ? 'متوسط' : 'جيد'}
                            </span>
                          </span>
                        </td>
                      </motion.tr>
