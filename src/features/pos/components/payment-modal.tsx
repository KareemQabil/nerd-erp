...
import { CartItem, PaymentMethod, OrderType, Customer, Table, Discount, BillSplit } from '../types/pos.types';
import { X, Check, CreditCard, Banknote, Smartphone, Wallet, Printer, ChefHat, Scissors, ArrowLeft, Receipt } from 'lucide-react';
...
interface PaymentModalProps {
  ...
  onPaymentComplete: (method: PaymentMethod, amount: number, change?: number, split?: BillSplit) => void;
  onPrintReceipt: () => void;
...
const PAYMENT_METHODS: { id: PaymentMethod; name: string; nameEn: string; icon: React.ReactNode; color: string }[] = [
  { id: 'cash', name: 'نقدي', nameEn: 'Cash', icon: <Banknote className="w-8 h-8" />, color: 'from-[#10b981] to-[#059669]' },
  { id: 'visa', name: 'فيزا', nameEn: 'Visa', icon: <CreditCard className="w-8 h-8" />, color: 'from-[#3b82f6] to-[#2563eb]' },
  { id: 'mada', name: 'مدى', nameEn: 'Mada', icon: <CreditCard className="w-8 h-8" />, color: 'from-[#22d3ee] to-[#0891b2]' },
  { id: 'stcpay', name: 'STC Pay', nameEn: 'STC Pay', icon: <Smartphone className="w-8 h-8" />, color: 'from-[#a855f7] to-[#7c3aed]' },
  { id: 'tabby', name: 'تابي', nameEn: 'Tabby', icon: <Wallet className="w-8 h-8" />, color: 'from-[#f59e0b] to-[#d97706]' },
  { id: 'tamara', name: 'تمارا', nameEn: 'Tamara', icon: <Wallet className="w-8 h-8" />, color: 'from-[#ec4899] to-[#db2777]' },
];
...
export function PaymentModal({
  ...
  const renderReviewStep = () => (
    <div className="space-y-6">
      ...
      <div className="bg-[rgba(255,255,255,0.03)] rounded-2xl p-4 border border-[rgba(255,255,255,0.1)]">
        <h3 className="text-lg font-['Almarai'] font-bold text-[#e2e2e6] mb-3" dir="auto">
          ملخص الطلب
        </h3>
        ...
        <div className="space-y-2 max-h-48 overflow-y-auto mb-4">
          {items.map((item, index) => (
            <div key={index} className="flex items-center justify-between py-2 border-b border-[rgba(255,255,255,0.05)]">
              <div className="flex-1">
                <p className="text-sm font-['Almarai'] text-[#e2e2e6]" dir="auto">
                  {item.product.name}
                </p>
                {item.modifiers && item.modifiers.length > 0 && (
                  <p className="text-xs text-[#c2c7ce] font-['Almarai']" dir="auto">
                    {item.modifiers.map(m => m.name).join(', ')}
                  </p>
                )}
              </div>
              <div className="text-right mr-4">
                <p className="text-sm font-['Arial'] text-[#c2c7ce]">×{item.quantity}</p>
              </div>
        ...
        <div className="space-y-2 pt-3 border-t border-[rgba(255,255,255,0.1)]">
          <div className="flex justify-between text-sm">
            <span className="font-['Almarai'] text-[#c2c7ce]" dir="auto">نوع الطلب:</span>
            <span className="font-['Almarai'] text-[#e2e2e6]" dir="auto">
              {orderType === 'dineIn' ? 'في المطعم' : orderType === 'takeaway' ? 'سفري' : 'توصيل'}
            </span>
          </div>
          {table && (
            <div className="flex justify-between text-sm">
              <span className="font-['Almarai'] text-[#c2c7ce]" dir="auto">الطاولة:</span>
              <span className="font-['Almarai'] text-[#e2e2e6]" dir="auto">{table.name}</span>
            </div>
          )}
          {customer && (
            <div className="flex justify-between text-sm">
              <span className="font-['Almarai'] text-[#c2c7ce]" dir="auto">العميل:</span>
              <span className="font-['Almarai'] text-[#e2e2e6]" dir="auto">{customer.name}</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-gradient-to-r from-[rgba(34,211,238,0.1)] to-[rgba(0,99,153,0.1)] border border-cyan-400/30 rounded-2xl p-4">
        <div className="space-y-2">
          <div className="flex justify-between">
            <span className="text-sm font-['Almarai'] text-[#c2c7ce]" dir="auto">المجموع الفرعي:</span>
            <span className="text-sm font-['Arial'] text-[#e2e2e6]">{subtotal.toFixed(2)} ر.س</span>
          </div>
          {discount > 0 && (
            <div className="flex justify-between">
              <span className="text-sm font-['Almarai'] text-[#c2c7ce]" dir="auto">
                الخصم {appliedDiscount ? `(${appliedDiscount.discount.name})` : ''}:
              </span>
              <span className="text-sm font-['Arial'] text-green-400">-{discount.toFixed(2)} ر.س</span>
            </div>
          )}
          <div className="flex justify-between">
            <span className="text-sm font-['Almarai'] text-[#c2c7ce]" dir="auto">الضريبة (15%):</span>
            <span className="text-sm font-['Arial'] text-[#e2e2e6]">{tax.toFixed(2)} ر.س</span>
          </div>
          <div className="flex justify-between pt-2 border-t border-cyan-400/30">
            <span className="font-['Almarai'] font-bold text-[#e2e2e6]" dir="auto">الإجمالي:</span>
            <span className="text-2xl font-['Arial'] font-bold text-[#99f0ff]">{total.toFixed(2)} ر.س</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        >
          ...
          <span className="font-['Almarai']" dir="auto">تقسيم الفاتورة</span>
        </button>
        <button
        ...
        >
          ...
          <span className="font-['Almarai'] font-bold" dir="auto">متابعة للدفع</span>
        </button>
      </div>
    </div>
  );
  ...
  const renderSplitStep = () => (
    <div className="space-y-6">
      ...
      >
        ...
        <span className="font-['Almarai']" dir="auto">رجوع</span>
      </button>

      <div>
        <h3 className="text-lg font-['Almarai'] font-bold text-[#e2e2e6] mb-4" dir="auto">
          اختر طريقة التقسيم
        </h3>
        ...
        <div className="space-y-3">
          ...
          >
            <div className="flex items-center justify-between">
              <div className="text-right">
                <p className="font-['Almarai'] font-bold text-[#e2e2e6] mb-1" dir="auto">
                  تقسيم متساوي
                </p>
                <p className="text-sm font-['Almarai'] text-[#c2c7ce]" dir="auto">
          ...
          {splitType === 'equal' && (
            ...
            >
              ...
              <div className="mt-4 p-3 bg-cyan-400/10 rounded-lg">
                <p className="text-sm font-['Almarai'] text-[#c2c7ce] text-center" dir="auto">
                  كل شخص يدفع: <span className="text-lg font-['Arial'] font-bold text-cyan-400">{(total / splitPeople).toFixed(2)} ر.س</span>
                </p>
              </div>
              <button
  ...
  const renderPaymentStep = () => (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-[rgba(34,211,238,0.1)] to-[rgba(0,99,153,0.1)] border border-cyan-400/30 rounded-2xl p-6 text-center">
        ...
        {billSplit && (
          <p className="text-xs text-[#c2c7ce] font-['Almarai'] mt-2" dir="auto">
            مقسم على {billSplit.parts.length} {billSplit.parts.length === 2 ? 'شخصين' : 'أشخاص'}
          </p>
        )}
      </div>
      ...
      {selectedMethod === 'cash' && (
        ...
        >
          ...
          {/* Amount Input */}
          <div>
            <label className="text-sm font-['Almarai'] text-[#c2c7ce] mb-2 block" dir="auto">
  ...
  const renderCompleteStep = () => (
    <div className="text-center py-8">
      ...
      <h3 className="text-2xl font-['Almarai'] font-bold text-[#e2e2e6] mb-2" dir="auto">
        تمت العملية بنجاح
      </h3>
      ...
      <div className="space-y-3">
        ...
        >
          ...
          <span className="font-['Almarai'] font-bold" dir="auto">طباعة الفاتورة</span>
        </button>
        ...
        {orderType === 'dineIn' && table && (
          ...
          >
            ...
            <span className="font-['Almarai']" dir="auto">إرسال للمطبخ</span>
          </button>
        )}
        ...
        >
          <span className="font-['Almarai']" dir="auto">إغلاق</span>
        </button>
      </div>
    </div>
  );
