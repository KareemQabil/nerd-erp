...
import { CartItem, PaymentMethod, OrderType, Customer, Table, Discount } from '../types/pos.types';
import { 
  X, Check, CreditCard, Banknote, Smartphone, Wallet, 
  ChevronLeft, Calculator, AlertCircle, Zap 
} from 'lucide-react';
...
export function PaymentModalRedesigned({
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-6 pb-28" dir={isRTL ? 'rtl' : 'ltr'}>
        {/* Backdrop */}
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
                    ...
                    <div className="bg-gradient-to-br from-cyan-500/20 to-blue-600/20 border border-cyan-400/30 rounded-2xl p-6">
                      <div className="text-center">
                        ...
                        <div className="flex items-center justify-center gap-6 text-sm">
                          <div className="flex items-center gap-2">
                            <span className="text-[#c2c7ce] font-['Almarai']" dir="auto">
                              {isRTL ? 'الضريبة:' : 'Tax:'}
                            </span>
                            <span className="font-['Inter'] text-[#e2e2e6]">{tax.toFixed(2)} ر.س</span>
                          </div>
                          {discount > 0 && (
                            <div className="flex items-center gap-2">
                              <span className="text-[#c2c7ce] font-['Almarai']" dir="auto">
                              ...
                              <span className="font-['Inter'] text-green-400">-{discount.toFixed(2)} ر.س</span>
                            </div>
                          )}
                        </div>
                      </div>
                        ...
                        {PAYMENT_METHODS.map((method) => (
                          ...
                          >
                            ...
                            <div className="relative h-full flex flex-col items-center justify-center gap-2 p-4">
                              ...
                              <span className="text-sm font-['Almarai'] font-bold text-[#e2e2e6] text-center leading-tight" dir="auto">
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="relative h-full flex items-center justify-between p-6">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="text-sm font-['Almarai'] text-[#c2c7ce] mb-1" dir="auto">
                              {isRTL ? 'طريقة الدفع' : 'Payment Method'}
                            </p>
                            <p className="text-xl font-['Almarai'] font-bold text-white" dir="auto">
                              {isRTL ? selectedMethodData?.nameAr : selectedMethodData?.nameEn}
                            </p>
                          </div>
                        </div>
                    ...
                    {selectedMethodData?.requiresCashInput && (
                      <div className="space-y-4">
                        {/* Quick Cash Buttons */}
                        <div>
                          <label className="block text-sm font-['Almarai'] font-bold text-[#e2e2e6] mb-3" dir="auto">
                          ...
                          <div className="grid grid-cols-4 gap-2">
                            {QUICK_CASH_AMOUNTS.map((amount) => (
                              ...
                              >
                                <span className="font-['Inter'] font-bold text-[#e2e2e6]">{amount}</span>
                              </button>
                            ))}
                          </div>
                        </div>
                        ...
                        <div>
                          <label className="block text-sm font-['Almarai'] font-bold text-[#e2e2e6] mb-3" dir="auto">
                            {isRTL ? 'أدخل المبلغ المستلم' : 'Enter Received Amount'}
                          </label>
                          ...
                          >
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-2">
                                ...
                                <span className="font-['Almarai'] font-bold" style={{ color: cashAmount >= total ? '#10b981' : '#ef4444' }} dir="auto">
                                  {isRTL ? 'الباقي' : 'Change'}
                                </span>
                              </div>
                              <span 
                                className="text-2xl font-['Inter'] font-bold"
                                style={{ color: cashAmount >= total ? '#10b981' : '#ef4444' }}
                              >
                                {cashAmount >= total ? change.toFixed(2) : (total - cashAmount).toFixed(2)} ر.س
                              </span>
                            </div>
                          </motion.div>
                        )}
                    ...
                    {!selectedMethodData?.requiresCashInput && (
                      <div className="bg-[rgba(34,211,238,0.1)] border border-[rgba(34,211,238,0.2)] rounded-xl p-4 flex items-start gap-3">
                        ...
                        <div>
                          <p className="text-sm font-['Almarai'] font-bold text-cyan-400 mb-1 text-right" dir="auto">
                            {isRTL ? 'دفع إلكتروني' : 'Electronic Payment'}
                          </p>
                          <p className="text-xs text-cyan-400/80 font-['Almarai'] leading-relaxed text-right" dir="auto">
                            {isRTL
                    ...
                    >
                      {paymentSuccess ? (
                        <>
                          ...
                          <span dir="auto">{isRTL ? 'تم الدفع بنجاح' : 'Payment Successful'}</span>
                        </>
                      ) : isProcessing ? (
                        <>
                          <motion.div
                          ...
                          <span dir="auto">{isRTL ? 'جاري المعالجة...' : 'Processing...'}</span>
                        </>
                      ) : (
                        <>
                          <Check className="w-6 h-6" />
                          <span dir="auto">{isRTL ? 'إتمام الدفع' : 'Complete Payment'}</span>
                        </>
                      )}
                    </button>
                  </motion.div>
