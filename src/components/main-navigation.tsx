...
export function MainNavigation() {
  ...
  return (
    <div className="fixed right-0 top-0 bottom-0 w-20 bg-linear-to-b from-[#023047] to-[#001219] border-l border-[rgba(255,255,255,0.1)] shadow-2xl z-50 flex flex-col">
      {/* Logo */}
        ...
        <div className="space-y-2 px-2">
          {navigationItems.map((item) => (
            <motion.button
              ...
              className={`relative w-full h-16 rounded-2xl flex flex-col items-center justify-center gap-1 transition-all group ${
                isActive(item.path)
                  ? `bg-linear-to-b ${item.color} text-[#00373a] shadow-lg`
                  : "bg-[rgba(255,255,255,0.05)] hover:bg-[rgba(255,255,255,0.1)] text-[#c2c7ce]"
              }`}
            >
              {item.icon}
              <span
                className="text-[9px] font-['Almarai'] font-bold text-center leading-tight"
                dir="auto"
              >
                {(isRTL ? item.name : item.nameEn).split(" ")[0]}
              </span>
              ...
              {/* Tooltip */}
              <div className="absolute left-full ml-3 hidden group-hover:block pointer-events-none">
                <div className="bg-[#1a1c1e] border border-cyan-400 rounded-lg px-3 py-2 shadow-xl">
                  <p
                    className="text-sm font-['Almarai'] text-[#e2e2e6] whitespace-nowrap"
                    dir="auto"
                  >
      ...
      <div className="border-t border-[rgba(255,255,255,0.1)] p-2 space-y-2">
        ...
        <div className="flex flex-col gap-2 mb-2">
          <ThemeSwitcher className="w-full h-10" />
          <LanguageSwitcher className="w-full h-10" />
        </div>

        {/* User Avatar */}
        <div className="flex flex-col items-center">
          <div className="w-12 h-12 rounded-full bg-linear-to-br from-cyan-400 to-[#006399] border-2 border-cyan-400 overflow-hidden">
            {user?.avatar ? (
              <img
                src={user.avatar}
                alt={user.name}
          ...
          <p
            className="text-[8px] font-['Almarai'] text-[#c2c7ce] mt-1 text-center leading-tight"
            dir="auto"
          >
            {user?.name.split(" ")[0]}
          </p>
