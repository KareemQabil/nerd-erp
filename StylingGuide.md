...
- **Leverage CSS Variables (if extending Tailwind):** For more complex theming scenarios or when defining custom color palettes, extend Tailwind's configuration to use CSS variables. This allows for dynamic theme changes without recompiling CSS.
  ...
  _Example (in `tailwind.config.js` - conceptual):_
...
## 8. Using CSS Variables for Theming

To ensure consistency across the application and support multiple themes (light, dark, luxury, etc.), always use the defined CSS variables instead of hardcoded hex values.
...
| `--on-primary`             | Text color on top of primary color       |
| `--primary-container`      | Tonal container for primary color        |
| `--on-primary-container`   | Text color on top of primary container   |
| `--secondary`              | Secondary brand color                    |
| `--on-secondary`           | Text color on top of secondary color     |
| `--secondary-container`    | Tonal container for secondary color      |
| `--on-secondary-container` | Text color on top of secondary container |
| `--background`             | Page background color                    |
| `--on-background`          | Text color on background                 |
...
<div style={{ backgroundColor: '#00acc1', color: '#ffffff' }}>
